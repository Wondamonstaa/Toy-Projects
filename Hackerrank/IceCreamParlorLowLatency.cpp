...
std::vector<int> icecreamParlor(int m, std::vector<int>& arr) {
    ...
    for (int i = 0; i < arr.size(); ++i) {
        ...
        if (it != valueToIndex.end()) {
            // Found a pair that sums to m
            return {it->second + 1, i + 1};  // Adding 1 to convert from 0-based index to 1-based index
        }

        // Insert the current value and its index into the hash table
